    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            title: Text(item.expression, style: const TextStyle(fontWeight: FontWeight.w600)),
            subtitle: Text('= ${item.result}'),
            trailing: item.timestamp != null
                ? Text('${item.timestamp}', style: TextStyle(color: Theme.of(context).hintColor, fontSize: 12))
                : null,
          ),
        );
      },
